...
// let stars = 5 //맨 윗줄의 별 개수(5개)
// let html = ''
// for (i = 1; i <= stars; i++){ // i -> 줄 번호(위부터 1~)
//     for (n = stars; n >= i; n--){ //n -> 별 찍을 횟수
//         html += '*'    
//     }
//     html += '<br>'
// }
...
//     if (userNames[index].includes('솔')) {
//         console.log(`이름에 '솔' 들어간 사람: ${userNames[index]}`)
//         break
//     }
//     userIndex++
// }
...
//     let check = new_Numbers.indexOf(numbers[i])
//     if (check == -1) new_Numbers.push(numbers[i]);
// }
...
// if(products.includes(상품명)){
//     let productIndex = products.indexOf(상품명)
    
//     let 수량 = Number(prompt("수량 입력"))
//     if (stock[productIndex] >= 수량) {
//         stock[productIndex] -= 수량 
//         console.log(`${상품명} 판매 완료! 남은 재고: ${stock[productIndex]}`)
//     } else {
...
//     } else{
//         usageMinutes[i] -= 30
//         fee = 1000 + 500 * parseInt(usageMinutes[i] / 10)
//         if(fee>20000){fee = 20000}
//         usageMinutes[i] += 30
//     }
//     parkingFees.push(fee)
// }
// console.log(parkingFees)

// for (let i = 0; i <= carNumbers.length - 1; i++){
//     html += `${carNumbers[i]}: ${usageMinutes[i]}분 주차, 최종 요금: ${parkingFees[i]} <br>`
// }
