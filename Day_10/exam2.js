...
function getWelcomeMessage(str) {
    return `환영합니다, ${str}님!`
}
let result1 = getWelcomeMessage('임준희')
console.log(result1)
...
let counter = 0
function increaseCount(){counter += 1;}
increaseCount()
increaseCount()
increaseCount()
console.log(counter)
...
printLocal() // 전역변수, 지역변수 모두 콘솔에 출력됨

console.log(globalText) // 전역변수는 호출 가능
// console.log(localText) // 에러, 지역변수는 바깥에서 호출 불가