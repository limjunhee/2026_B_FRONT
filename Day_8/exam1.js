...
const me = { name: '임준희' , age: 25, city: '안양'}
console.log(me)
...
let values = Object.values(book) //객체 내 모든 속성값을 배열로 반환하기
...
const person = { name: '김코딩', age: 25, };
person.email = 'abcd1234@gmail.com'
delete person.age
console.log(person)
...
const country = { name: '대한민국', capital: '서울', population: 51780000 };
let keyArray = Object.keys(country)
let valueArray = Object.values(country)
...
for (let i = 0; i <= inventoryKeys.length - 1; i++){
    if (inventoryValues[i] < 10) { console.log(inventoryKeys[i]) }
}
// 또는
for (let i = 0; i <= inventoryKeys.length - 1; i++) {
    let values = inventoryKeys[i] // i번째 속성 이름 뽑아내기
    if (inventory[values] < 10) { // i번째 속성값이 10 미만이면
        console.log(values)
    }
}
/* 예제 10: 객체 배열 순회하기
...
*/
let response = { "currentCount": 10, "data": [{ "데이터기준일자": "2026-01-31", "동별": "부평1동", "세대수": 17300, "인구수(계)": 34448, "인구수(남)": 16521, "인구수(여)": 17927 }, { "데이터기준일자": "2026-01-31", "동별": "부평2동", "세대수": 8410, "인구수(계)": 16635, "인구수(남)": 8202, "인구수(여)": 8433 }, { "데이터기준일자": "2026-01-31", "동별": "부평3동", "세대수": 7416, "인구수(계)": 14860, "인구수(남)": 7361, "인구수(여)": 7499 }, { "데이터기준일자": "2026-01-31", "동별": "부평4동", "세대수": 17639, "인구수(계)": 32852, "인구수(남)": 16165, "인구수(여)": 16687 }, { "데이터기준일자": "2026-01-31", "동별": "부평5동", "세대수": 16395, "인구수(계)": 32894, "인구수(남)": 16151, "인구수(여)": 16743 }, { "데이터기준일자": "2026-01-31", "동별": "부평6동", "세대수": 9295, "인구수(계)": 19045, "인구수(남)": 9382, "인구수(여)": 9663 }, { "데이터기준일자": "2026-01-31", "동별": "산곡1동", "세대수": 6268, "인구수(계)": 14252, "인구수(남)": 7016, "인구수(여)": 7236 }, { "데이터기준일자": "2026-01-31", "동별": "산곡2동", "세대수": 10606, "인구수(계)": 28647, "인구수(남)": 14006, "인구수(여)": 14641 }, { "데이터기준일자": "2026-01-31", "동별": "산곡3동", "세대수": 8505, "인구수(계)": 19711, "인구수(남)": 9495, "인구수(여)": 10216 }, { "데이터기준일자": "2026-01-31", "동별": "산곡4동", "세대수": 6534, "인구수(계)": 16222, "인구수(남)": 7704, "인구수(여)": 8518 }], "matchCount": 22, "page": 1, "perPage": 10, "totalCount": 22 }
...
for( let index = 0; index <= data.length - 1; index++){
    let 동별객체 = data[index]; // index번째 동별 객체 꺼내기(순회)
    // 주의할 점: 반복문 안에서 변수 선언 시 선언이 반복되면서 선언됨, 즉 선언/초기화가 계속 반복된다.
    html += `<tr>
                <td> ${동별객체.동별} </td>
                <td> ${동별객체.세대수} </td>
                <td> ${동별객체["인구수(계)"]} </td>
